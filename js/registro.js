...
function validarEmail(x) {
  if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(x.value)) {
    console.log("re bien ese email, mi rei ");
    x.className = "form-control rounded-pill border-1 is-valid";
    return true;
  } else {
    console.log("re mal ese email, fijate pa");
    x.className = "form-control rounded-pill border-1 is-invalid";
    return false;
  }
}
